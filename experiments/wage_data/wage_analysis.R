...
data$log_wage = log(data$salary / (data$weeks_worked * data$hours_worked * 5))
...
ggplot(data, aes(log_wage)) +
  ...
  coord_cartesian(xlim=c(-0.9, 4.2)) +
  theme_light()+
...
quantile_male = wtd.quantile(x=data$salary, weights=data$plotweight*(data$sex=='male'), normwt=TRUE, probs=0.5)
quantile_female = wtd.quantile(x=data$salary, weights=data$plotweight*(data$sex=='female'), normwt=TRUE, probs=0.5)
quantile_female/quantile_male
...
#save(mrf_fit, train_idx, data, X, Y, file='~/Documents/projects/heterogeneity/wage_data/computed_data')
...
for(i in sample((1:nrow(X))[-train_idx], 50)){
  ...
  gg = ggplot(plotdf, aes(log_wage)) +
    ...
    theme(legend.position = c(0.83, 0.5),
          ...
          axis.title.x = element_text(size=15),
          axis.title.y = element_text(size=15))+
    annotate("text", x=-1.14, y=Inf, hjust=0, vjust=1, size=5, label = point_description(data[i,]))
  plot(gg)
  ggsave('~/Documents/projects/heterogeneity/paper/teacher.png', width=16, height=11, units='cm')
}
###########################################################
