univariateComparison <- function(dataset = "synthetic1", 
  ...
  ids.train <- sample(1:n, size = test.frac*n, replace = FALSE) 
  ...
  if (dataset %in% c("synthetic1","synthetic2","synthetic3","synthetic4")) {
    ...
    qQRF <- predict(qRF, data = nd, quantiles = c(.1, .9), type = "quantiles")
    qDRF <- predict(dRF, newdata = cbind(n, X), functional = "quantile", quantiles = c(0.1,0.9))$quantile
    qGRF <- predict(gRF, newdata = cbind(n, X), quantiles = c(0.1,0.9))
    qTRF <- list(p1 = predict(trf, newdata = nd, type = "quantile", prob = .1),
      ...
      if (dataset == "synthetic1") {
        ...
        q1 <- ifelse(d$X[ids.train,1] >= .5, qt(.1, df = 3), qnorm(.1))
        q9 <- ifelse(d$X[ids.train,1] >= .5, qt(.9, df = 3), qnorm(.9))
        q1.train <- ifelse(d$X[ids.train,1] >= .5, qt(.1, df = 3), qnorm(.1))
        q9.train <- ifelse(d$X[ids.train,1] >= .5, qt(.9, df = 3), qnorm(.9))
    ...
    if (verbose) {
      ...
      colB <- rgb(0, 0, 0.75, .8)
      plot(d$X[ids.train,1], d$y[ids.train], pch = 19, col = c(col, colRlight)[(d$y[ids.train] < q1.train | d$y[ids.train] > q9.train) + 1], cex = 1)
      lines(x, q1, lty = 2, lwd = lwd * 1.5, col = "black", type = "S")
      lines(x, q9, lty = 2, lwd = lwd * 1.5, col = "black", type = "S")
      lines(x, qQRF$predictions[,1], lty = 1, lwd = lwd * 1.5, col = "red", type = "S")
      lines(x, qQRF$predictions[,2], lty = 1, lwd = lwd * 1.5, col = "red", type = "S")
      lines(x, qDRF[,1,1],lty = 1, lwd = lwd * 1.5, col = "darkgreen", type = "S")
      lines(x, qDRF[,1,2],lty = 1, lwd = lwd * 1.5, col = "darkgreen", type = "S")
  ...
  losses <- lapply(1:length(quantiles.grid), function(i) {
    if (dataset %in% c("synthetic1","synthetic2","synthetic3","synthetic")) {
      q.loss <- c(mean(qLoss(y = d$y[ids.test], q = q.test[[i]], alpha = quantiles.grid[i])),
                  ...
                  mean(qLoss(y = d$y[ids.test], q = unlist(qTRF.test[[i]]), alpha = quantiles.grid[i])))
      names(q.loss) <- c("truth", "qrf", "drf", "grf", "trf")
    } else {
