...
if (PREPRO == "futureLogReturns") {
  etfs <- apply(etfs, 2, function(x) log(shift(x, n = 1, type = "lag")/x))
  etfs <- etfs[-1,]
  date <- date[-1]
} else if (PREPRO == "futureLogReturns") {
  etfs <- apply(etfs, 2, function(x) log(shift(x, n = -1, type = "lead")/x))
  etfs <- etfs[-nrow(etfs),]
  date <- date[-nrow(date)]
}
...
Y <- as.matrix(etfs[,c(1:11)])
X <- as.matrix(etfs[,-c(1:11)])
